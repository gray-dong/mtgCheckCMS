...
layui.define(['layer', 'laytpl', 'upload','form'], function(exports){
  var v = '3.8.0';
  var $ = layui.$;
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ...
    ,'{{# layui.each(d.friend, function(index, item){ var spread = d.local["spread"+index]; }}'
      ,'<li>'
        ,'<h5 layim-event="spread" lay-type="{{ spread }}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.groupname||"我的好友" }}</span><em>(<cite class="layim-count"> {{ (item.list||[]).length }}</cite>)</em></h5>'
    ...
    ,'<ul class="layui-unselect layim-tab-content {{# if(!d.base.isfriend && d.base.isgroup){ }}layui-show{{# } }}">'
      ,'<li>'
        ...
        ,'</ul>'
      ,'</li>'
      ,'<li layim-event="addGroup" data-type="" data-index="" class="addGroup"><span class="layui-icon">&#xe654;新建群聊</span></li>'
    ,'</ul>'
    ,'<ul class="layui-unselect layim-tab-content  {{# if(!d.base.isfriend && !d.base.isgroup){ }}layui-show{{# } }}">'
      ,'<li>'
    ...
    ,'<ul class="layui-unselect layui-layim-tool">'
      ...
      // ,'<li class="layui-icon layim-tool-addGroup" layim-event="addGroup" title="新建群聊">&#xe654;新建群聊</li>'
      ,'{{# if(d.base.msgbox){ }}'
  ...
  var banRightMenu = function(){
    ...
    layimMain.find('.layim-list-history').on('contextmenu', 'li', function(e){
      var othis = $(this);
      debugger
      var html = '<ul data-id="'+ othis[0].className +'" data-index="'+ othis.data('index') +'"><li layim-event="menuHistory" data-type="one">移除该会话</li><li layim-event="menuHistory" data-type="all">清空全部会话列表</li></ul>';
    ...
    layimMain.find('.layim-list-group').on('contextmenu', 'li', function(e){
      var othis = $(this);
      // var oclass = othis[0].className;
      // var index = $.trim(oclass.substr(6));
      var html = '<ul data-id="'+ othis[0].className +'" data-index="'+ othis.data('index') +'"><li layim-event="menuGroup" data-type="sendMsg">发消息</li><li layim-event="menuGroup" data-type="quitGroup">退出该群</li></ul>';
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,skin: 'layui-box layui-layim layui-layim-close'
      ,shade: false
      ,closeBtn: false
      ,anim: 2
      ,offset: 'rb'
  ...
  var anim = 'layui-anim-upbit', events = {
      ...
      if(othis.data('down')){
        ...
        events.groupMembers.index = layer.tips('<ul class="layim-members-list"></ul>', othis, {
          ...
          ,success: function(layero){
            ...
            var _roomID = members.data.id;
            post(members, function(res){
              layui.each(res.list, function(index, item){
                li += '<li data-uid="'+ item.id +'"><a href="javascript:;"><img src="/src/style/res/微信图片_20181210105316.png"><cite>'+ item.username +'</cite></a></li>';//'+ item.avatar +'
                membersCache[item.id] = item;
              });
              li += '<li layim-event="addGroupNode" layim-roomID="'+_roomID+'"><a href="javascript:;"><i class="layui-icon" style="display:inline-block;line-height:48px;font-size:18px;">&#xe654;</i><cite>添加成员</cite></a></li>';
              ul.html(li);
    ...
    ,menuHistory: function(othis, e){
      ...
      if(type === 'one'){
        ...
        $( '.layim-list-history .'+parent.data('id')).remove();
    ...
    // 群组右键操作
    ,menuGroup: function(othis, e){
      var local = layui.data('layim')[cache.mine.id] || {};
      var parent = othis.parent(), type = othis.data('type');
      var groupElem = layimMain.find('.layim-list-group');
      var none = '<li class="layim-null">暂无群组</li>'
      var hisElem = layimMain.find('.layim-list-history');
      var hisNone = '<li class="layim-null">暂无历史会话</li>'

      if(type === 'sendMsg'){
        // debugger;
        // console.log($('.layim-list-group .'+parent.data('index')));
        // console.log($(parent.data('id')));
        $('.layim-list-group .'+parent.data('id')).click();
      } else if(type === 'quitGroup') {
        // console.log(parent.data('index'));
        var list = parent.data('index');
        cache['group'].splice(list,1);
        ...
        $('.layim-list-group .'+parent.data('id')).remove();
        $('.layim-list-history .'+parent.data('id')).remove();
        if(groupElem.find('li').length === 0){
          groupElem.html(none);
        }
        if(hisElem.find('li').length === 0){
          hisElem.html(hisNone);
        }
      }
    ...
    ,addGroup: function () {
      ...
      layer.open({
        type: 1 
          ...
          ,success: function(layero){
            layui.view(this.id).render('contacts/addGroup').done(function(){
              var _input = "";
              $.each(asd,function(key,item){
                _input += '<input type="checkbox" name="'+key+'" lay-skin="primary" title="'+item+'">';
              })
              $('.groupUser').html(_input);
              form.render(null, 'addGroupTab');
              form.on("submit(addGroupBtn)",function (data) {
    ...
    ,addGroupNode:function(){
      ...
      layer.open({
        type: 1 
          ...
          ,zIndex: layer.zIndex 
          ,success: function(layero){
            layui.view(this.id).render('contacts/addGroupNode').done(function(){
              var _input = "";
              $.each(friends,function(index,item){
                _input += '<input type="checkbox" name="'+item.id+'" lay-skin="primary" title="'+item.username+'">';
              })
              $('.groupUser').html(_input);
              form.render(null, 'addGroupTab');
              form.on("submit(addGroupBtn)",function (data) {
